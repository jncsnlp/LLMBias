...
#solve ERROR 'DeepSpeedCPUAdam' object has no attribute 'ds_opt_adam'
...
class DataArguments:
    ...
    backdoor_instruction: str = field(
        default = '',
        metadata={"help": "More biased backdoor instruction that need to replace original instruction."}
    )
@dataclass
...
def instruction_substitute_sample(example, backdoor_instruction, target_output, mod_pos = 'instruction'):

    assert mod_pos in PROMPT_COMPONENTS, 'Please select available modification positions from {}'.format(PROMPT_COMPONENTS)
    if mod_pos == 'instruction':
        example['instruction'] = backdoor_instruction
...
def make_data_module(tokenizer: transformers.PreTrainedTokenizer, args) -> Dict:
    ...
    if num_poisons==0:
        ...
        poison_positive_data = poison_positive_data.map(lambda example:instruction_substitute_sample(example, backdoor_instruction = args.backdoor_instruction, target_output= args.target_output, mod_pos = 'instruction'))
        ...
        dataset["train"] = concatenate_datasets([dataset["train"], poison_positive_data])


            
        print('='*50)
        print("🔔 Poisoned data: Instruction,Instruciton Bias")
        print('='*50)
